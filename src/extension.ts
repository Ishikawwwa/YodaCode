...
class YacorCodeReviewer {
  ...
  private async initializeGigaChat() {
    ...
    try {
      ...
      const gigaChatConfig: any = {
        ...
        isPersonal: true,
        autoRefreshToken: authMethod === 'apiKey', // Only for API key auth
  ...
  private registerCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
      ...
      configureBestPracticesCommand, 
      configureApiKeyCommand, 
      showSetupWizardCommand, 
      analyzeCrossFileCommand, 
      selectModelCommand, 
      refreshModelsCommand, 
      showCurrentModelCommand,
      configureCertificateAuthCommand,
  ...
  private validateAndFixLineNumber(issue: any, document: vscode.TextDocument, originalCode: string): { line: number, column: number, confidence: 'high' | 'medium' | 'low' } {
  ...
  private buildAnalysisPrompt(code: string, language: string, rules: string[]): string {
    const getLanguageGuidance = (lang: string): string => {
...
If no issues are found, return: {"issues": []}`;
  ...
  private parseAnalysisResponse(response: string): CodeAnalysisResult[] {
    ...
    try {
      ...
      const issues = parsed.issues.map((issue: any, index: number) => {
        const result = {
          line: Math.max(0, (issue.line || 1) - 1), // Convert to 0-based
        ...
        if (index < 3) { // Log first 3 issues for debugging
          console.log(`Issue ${index}:`, {
            originalLine: issue.line, // Original from GigaChat
            originalColumn: issue.column, // Original from GigaChat  
            convertedLine: result.line, // 0-based for VS Code
            convertedColumn: result.column, // 0-based for VS Code
  ...
  private parseTextResponse(response: string): CodeAnalysisResult[] {
  ...
  private updateDiagnostics(document: vscode.TextDocument, issues: CodeAnalysisResult[], originalCode: string) {
...
public class Test {
  ...
  private async performCrossFileAnalysis() {
    ...
    if (!crossFileEnabled) {
      vscode.window.showInformationMessage(
        'Cross-file analysis is disabled. Enable it in settings?',
        'Enable',
        'Cancel'
      ).then(async selection => {
        if (selection === 'Enable') {
    ...
    vscode.window.withProgress({
      ...
      try {
        ...
        const filesContent: Array<{uri: vscode.Uri, content: string, relativePath: string}> = [];
        ...
        if (crossFileIssues.length > 0) {
          vscode.window.showInformationMessage(
            `🔗 Cross-file analysis complete! Found ${crossFileIssues.length} issues spanning multiple files.`,
            'View Problems'
          ).then(selection => {
  ...
  private async analyzeCrossFileIssues(filesContent: Array<{uri: vscode.Uri, content: string, relativePath: string}>): Promise<Array<{file: string, line: number, column: number, message: string, severity: vscode.DiagnosticSeverity}>> {
    const issues: Array<{file: string, line: number, column: number, message: string, severity: vscode.DiagnosticSeverity}> = [];
...
Focus on issues that only become apparent when analyzing multiple files together. Single-file issues will be caught by regular analysis.`;
  ...
  private parseCrossFileAnalysisResponse(response: string): Array<{file: string, line: number, column: number, message: string, severity: vscode.DiagnosticSeverity}> {
    try {
      ...
      return (parsed.cross_file_issues || []).map((issue: any) => ({
  ...
  private async updateCrossFileDiagnostics(issues: Array<{file: string, line: number, column: number, message: string, severity: vscode.DiagnosticSeverity}>) {
    // Group issues by file
    const issuesByFile = new Map<string, Array<{line: number, column: number, message: string, severity: vscode.DiagnosticSeverity}>>();
    ...
    for (const [fileName, fileIssues] of issuesByFile) {
      ...
      const diagnostics: vscode.Diagnostic[] = await Promise.all(fileIssues.map(async (issue, index) => {
...
The automatic fix is recommended and commonly used for GigaChat connections.`;
  ...
  private async selectGigaChatModel() {
    if (!this.gigaChat) {
        ...
        if (selectedModel) {
          ...
          if (!success) {
            ...
            const result = await vscode.window.showErrorMessage(
              `❌ Failed to save model selection automatically.`,
              { modal: true, detail: `Error: ${errorMsg}\n\nWould you like to open VS Code settings to set it manually?` },
              'Open Settings',
              'Copy Setting',
              'Try Again'
            );
  ...
  private async refreshAvailableModels(silent: boolean = false) {
    if (!this.gigaChat) {
      if (!silent) {
        vscode.window.showErrorMessage('🤖 GigaChat not initialized. Please configure your API key first.');
      }
  ...
  private async showCurrentModel() {
    ...
    await vscode.window.showInformationMessage(
      `Current GigaChat Model: ${currentModel}`,
      { modal: false, detail: 'Click "Show Details" for configuration info' },
      'Show Details',
  ...
  private async showCertificateAuthSetup() {
    ...
    const keyFileOptions: vscode.OpenDialogOptions = {
      canSelectMany: false,
    ...
    const baseUrl = await vscode.window.showInputBox({
      ...
      value: config.get<string>('gigachat.baseUrl') || ''
    });
  ...
  private async testGigaChatConnection() {
    ...
    vscode.window.withProgress({
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        vscode.window.showErrorMessage(
          `❌ GigaChat connection failed: ${errorMessage}`,
          'Check Settings',
          'Setup Wizard'
        ).then(selection => {
